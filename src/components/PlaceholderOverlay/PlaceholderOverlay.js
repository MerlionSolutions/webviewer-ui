...
import './PlaceholderOverlay.scss';

const TOOL_NAME = 'AnnotationCreatePlaceholder';
const canvasWidth = 160;
const canvasHeight = 58;


class PlaceholderOverlay extends React.Component {
  static propTypes = {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.activeToolName !== this.props.activeToolName && this.props.activeToolName === TOOL_NAME) {
      ...
      this.props.openElement('placeholderOverlay');
  ...
  handleClickOutside = (e) => {
    ...
    if (this.props.isActive) {
      this.props.toggleElement('placeholderOverlay');
    }

    if (!rubberStampToolButton) {
      this.props.closeElement('placeholderOverlay');
...
const mapStateToProps = state => ({
  isDisabled: selectors.isElementDisabled(state, 'placeholderOverlay'),
  isOpen: selectors.isElementOpen(state, 'placeholderOverlay'),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTranslation()(onClickOutside(PlaceholderOverlay)));
